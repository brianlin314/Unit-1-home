...
def read_data(file_path, seen_attack_name, unseen_attack_name):
    ...
    df = df.drop(['Timestamp', 'Dst Port', 'Protocol'], axis=1)
    df = df.fillna(0)
    df = replace_inf(df)
    # print("origin df shape:", df.shape)
    scaler = joblib.load('/SSD/p76111262/CIC2018_csv/min_max_scaler_without_protocol.joblib')
    df_scaled_features = scaler.transform(df.drop(columns=['Label', 'Sub_Label']))
    
    df_scaled = pd.DataFrame(df_scaled_features, columns=df.columns.drop(['Label', 'Sub_Label']))
    df_scaled['Label'] = df['Label'].values
    df_scaled['Sub_Label'] = df['Sub_Label'].values
    ...
    # df_scaled_seen = df_scaled[df_scaled['Label'] != attack_name]
    # df_scaled_unseen = df_scaled[df_scaled['Label'] == attack_name]
...
def train(seen_and_unseen_df, seen_df, unseen_df,  num_epochs):
    ...
    X = seen_df.drop(['Label', 'Sub_Label'], axis=1) # 資料特徵
    y = seen_df['Sub_Label'] # 目標 label 

    X_unseen = unseen_df.drop(['Label', 'Sub_Label'], axis=1) # 資料特徵
    y_unseen = unseen_df['Sub_Label'] # 目標 label
...
if __name__ == '__main__':
    num_epochs = 100
    csv_file_path = '/SSD/p76111262/CIC2018_csv/CIC2018_attack_with_subset.csv'
    label_embeddings_file_path = '/SSD/p76111262/label_embedding_32'
    seen_attack_name = ['DDoS_HOIC', 'DDoS_LOIC-UDP', 'DoS_Slowloris', 'DoS_Hulk', 'DoS_SlowHTTPTest', 
                        'DoS_GoldenEye', 'BruteForce-XSS', 'BruteForce-Web', 'SQL-Injection', 'BruteForce-SSH',
                        'BruteForce-FTP', 'Infiltration', 'Botnet']
    unseen_attack_name = ['DDoS_LOIC-HTTP']
